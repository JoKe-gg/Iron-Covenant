...
<h3>
  ...
  <br> Choose your character and upgrades and try to eliminate each of the beasts on your road.
</h3> 
...
<ul> 
  <li> <a href="#Technical-Features"> Technical features </a> 
    <ul> 
      <li> <a href="#SO-DrivenArchitecture">Scriptable object-driven architecture</a> </li>
...
<a id="SO-DrivenArchitecture" ></a>
<h3 >Scriptable object-driven architecture  </h3>

<a id="Definition-SO"></a>
<h4 align="right"><em > from here SO - Scriptable object </em></h4> 


I have used SO assets to be able to edit and save data between scenes. It gives ability to reuse data for all instances of the certain class.

Also I have used SO for Save Manager to avoid widespread usage of Singletons and to avoid Null References due to Life cycles of Singleton.
...
<br>
Upgrades include effects for weapon and stats modifiers. It gives an ability to merge systems having only one source of data. The purpose of certain Upgrade is being set by enum list
...
* Enemy spawner:
  * Enemy spawner refers to pool has base of enemies prefabs, their spawn intervals, and type of spawned enemy (regular or boss).
    ...
    * type of enemy determines amount of created instances of enemy prefab in pool.
...
I have created my own pool system based on generic class:
...
I know that Unity Engine already has its own tool for pooling, but I wanted to practice in Generic Classes and to understand how it works inside.
...
I have realized Global State System that is based in singleton pattern. 

It keeps current state and calls event to notify each subscribed class that state has been changed.

It's very useful for scaling project. You don't need to disable or enable some features directly when it's required - you can just change state and realize disabling for features that are needed to be disabled.
...
I have troubles while I was testing when a lot of game objects were destroying fps dropped. It's caused by GC clearing heap.

Solution is Pooling or reusing game objects.
It gave me stable performance even when it's hundreds of enemies.
...
<h3> Upgrade scalability</h3>

At first I created Upgrade system where each upgrade layer had its own Upgrade SO. It was so hard to scale.

Solution is to rework for one SO. It has become easy to merge.
...
<a align="center" id="Technologies-Used"></a>
